...
import { href, Link, Links, useLocation } from "react-router-dom";
...
const Sidebar = () => {
  ...
  const navigationItems = [
    ...
    role === 'ADMIN' ?
    { name: "Employees", icon: UserIcon, href: "/employees" }:
    { name: "Attendance", icon: CalendarIcon, href: "/attendance" },
    { name: "Payslips", icon: DollarSignIcon, href: "/payslips" },
    { name: "Leave", icon: FileTextIcon, href: "/leave" },
  ...
  const sidebarContent = (
    <>
      ...
      <div className="px-5 pt-6 pb-5 border-b border-white/6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <p className="text-[11px] text-stale-500">Management System</p>
          </div>
          </div>
      ...
      <div className="flex-1 px-3 space-y-0.5 overflow-y-auto">
        {navigationItems.map((item)=> {
          ...
          return(
            ...
            <item.icon className={`w-[17px] h-[17px] shrink-0 ${isActive ? 'text-indigo-300' : 'text-indigo-400 group-hover:text-slate-300'}`}/>
      ...
      <div className="p-3 border-t border-white/6">
        ...
        className="flex items-center gap-3 w-full cursor-pointer px-3 py-2.5 rounded-md text-[13px] font-medium text-slate-400 hover:text-rose-400 hover:bg-rose-500/8 transition-all duration-150">
  ...
  return (
    <>
      ...
      {/* {Sidebar - mobile} */}
      <aside
        className={`lg:hidden fixed inset-y-0 left-0 w-72 bg-linear-to-b from-slate-900 via-slate-900 to-slate-950 text-white z-50 flex-col transform transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        {sidebarContent}
      </aside>
    </>
